...
fun ChineseScreenPreview() {
    ChineseScreenContent(
        state = ChineseState(searchState = SearchState.Loading),
        actor = {}
    )
}
...
private fun ChineseScreenContent(
    ...
    Column(
        ...
        when (val searchState = state.searchState) {
            ...
            is SearchState.Result -> PagedHanziBoard(state = searchState, actor = actor)
        }
    }
}

@Composable
private fun PagedHanziBoard(state: SearchState.Result, actor: (ChineseAction) -> Unit) {
    HanziBoard(
        hanziItems = state.items,
        showPrevious = state.hasPrevious,
        onPreviousClick = { actor(ChineseAction.ShowPrevious) },
        showNext = state.hasNext,
        onNextClick = { actor(ChineseAction.ShowNext) }
    )
}
...
@Composable
private fun TopBar(
    searchMode: SearchMode, actor: (ChineseAction) -> Unit
) {
    ...
    TopAppBar(
        ...
        IconMenu(
            ...
            onClick = {
                actor(ChineseAction.ChangeMode)
